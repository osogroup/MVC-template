...
const outputHTML = async () => {
  ...
  // -------------------------------------------------- Item Values --------------------------------------------------
    ...
    if (configTypeData.editable.includes(headerKey) == true)
    {
      ...
      itemHeader += '<br><input class="col-12" id="input'+headerKey+'" type="textarea" value="'+headerValue+'" onchange="localStorage.setItem('+headerKey+', '+document.getElementById('input'+headerKey)+');">';
    }
    else
    {
